...
At the moment, I have the following in my init.org file to tangle on save. I
would like to experiment with an async tangle through a custom function and
atleast a message once it is complete. Perhaps that can also include some tests
down the line.
...
This is inspired from [[https://www.reddit.com/r/emacs/comments/5ej8by/asynchronous_tangle_and_compile_of_configorg/][Asynchronous tangle and compile of config.org
(question/issue) : emacs]] on reddit. Since I am using straight.el, the byte
compilation of packages is not necessary (or already taken care of).
...
- Note taken on [2020-03-22 Sun 12:41] \\
  Apparently, with a high gloss screen, it makes no difference whether the theme
  is light or dark. The distracting reflkection still exists. Interestingly
  enough, the macbook pro screen does not reflect much under the same conditions
  and position with respect to light source.
...
*** TODO Plans on customising this theme [1/10]
...
- [X] Set the variable pitch, fixed pitch and other options to clearly
  distinguish code blocks from the text and the same formatting should be
  followed for major modes.
  - So far this seems to be working as expected in the theme. I am not sure
    whether it is a good idea to use variable width in terms of the delays in
    rendering the buffer. 
- [ ] Highlighting for the poet-dark theme should be a light red that is
  visible on a high gloss screen. THis highlighting should not interfere with
  the background color of code blocks. Perhaps an inverse could be specified
  only for the code blocks
- [ ] Headlines should not be in monospaced font. Text in monospaced font
  should be clearly distinguished from the monospaced code blocks.
...
- [ ] Change the font family from baskerville to something more palatable, but
  with the serif type. Using a serif type will make it easily distnguishable
...
:header-args: :tangle no
:END:
** Doct for org capture templates
...
This section contains sections to search in specific directories, with
customised options. 

*** Episteme search
Borrowed from ldlework
#+BEGIN_SRC emacs-lisp
...
*** Projects search
- [ ] Exclude git files and csv files
