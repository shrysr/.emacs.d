...
#+TITLE: shrysr's Dotemacs config

* Introduction

I've been using my own configuration over that of Scimax for the past many years. This is a reboot to reverse the bias, and in the process try to contribute to the reliability and modularity of Scimax.
...
** Tangle Emacs init on save 
*** Tangle on save without async
...
** Directory setups
*** Function to create directories based on home path 

- [ ] Test whether this is really OS independent.

The idea is that a home directory is constructed, and then various folders are just appended to the home directory path. 
...
[[https://github.com/progfolio/doct][DOCT]] makes it a lot easier to define capture templates in a clean manner. At
the moment, I am interested in adding hooks to specific functions and improving
...
(setq org-capture-templates
      (doct '(("Tasks" :keys "t"
	      ...
	      ("Notes" :keys "n"
               ...
               :prepend t
               :template :template (lambda () (sr/fun/todo-passive))
               :children (("Fast note"  
			   :keys "n"
			   :type entry	                           
			   :headline   "One"
...
- Note taken on [2020-03-22 Sun 12:41] \\
  Apparently, with a high gloss screen, it makes no difference whether the theme is light or dark. The distracting reflection still exists. Interestingly
  enough, the macbook pro screen does not reflect much under the same conditions and position with respect to light source.
...
*** TODO Plans on customising this theme [1/13]

Things I would like to customise with this theme

- [ ] Source block colors for main languages. This should be a very subtle
  shade that works both on terminal as well as GUI emacs.
- [X] Set the variable pitch, fixed pitch and other options to clearly
  distinguish code blocks from the text and the same formatting should be followed for major modes.
  - So far this seems to be working as expected in the theme. I am not sure
    whether it is a good idea to use variable width in terms of the delays in rendering the buffer.
- [ ] Highlighting for the poet-dark theme should be a light red that is visible on a high gloss screen. THis highlighting should not interfere with the background color of code blocks. Perhaps an inverse could be specified only for the code blocks
- [ ] Headlines, comments, source blocks, and code should be clearly distinguished.
- [ ] Headline levels should have different colors to enable quick distinguishing.
- [ ] Further customisation of the modeline to make it closer to spacemacs style.
- [ ] Parenthesis should be made dull and fading into the backgrond
- [ ] Rainbow mode should be implemented.
- [ ] Appropriate electric pair mode that does not interfere with yasnippet.
  - [ ] Perhaps this could be made to not pair arrow brackets < >.
- [ ] Headline levels should of different sizes and easily distinguishable
- [ ] Change the font family from baskerville to something more palatable, but
  with the serif type. Using a serif type will make it easily distnguishable from the code as well.
- [ ] incorporate the customisation functions in [[http://www.howardism.org/Technical/Emacs/orgmode-wordprocessor.html][Org as a Word Processor]].
- [ ] Further customisation explanations are available in [[https://zzamboni.org/post/beautifying-org-mode-in-emacs/][zzamboni.org | Beautifying Org Mode in Emacs]]

** Olivetti mode

This package is useful in centering the text in a buffer, and is particularly
helpful with wide screens.
...
(setq '(org-spacer-headline-blanks 1)
;; Number of spaces after non-headline elements
